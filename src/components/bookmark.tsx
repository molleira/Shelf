  ...
  useEffect(() => {
    const fetchBookmarksFromMarkdown = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(`Failed to fetch bookmarks.md: ${response.status} ${response.statusText}`)
        }
        ...
        setError(`Failed to load bookmarks: ${err instanceof Error ? err.message : String(err)}`)
      } finally {
        setLoading(false)
      }
    }
  ...
  if (error) {
    return (
      <div className="bookmarks-container">
        <div className="error">
          <p>{error}</p>
          <p className="error-hint">Make sure bookmarks.md exists in the public folder</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="bookmarks-container">
        <div className="empty">
          ...
          <p className="empty-hint">Make sure your bookmarks.md file is formatted correctly:</p>
          ...
          <div className="bookmarks-grid">
            {categoryBookmarks.map((bookmark, index) => (
              ...
              >
                ...
                <span className="bookmark-url">{new URL(bookmark.url).hostname.replace("www.", "")}</span>
              </a>
            ))}
          </div>
        </div>
